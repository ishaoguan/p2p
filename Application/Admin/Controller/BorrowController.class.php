...
class BorrowController extends ACommonController
{
    /**
    +----------------------------------------------------------
    * 默认操作
    ...
    */
	...
	public function waitverify_1()		//二审前台
    {
		//$this->sendemail();//yjy test
		$map=array();
		$map['b.borrow_status'] = 100;
		if(!empty($_REQUEST['uname'])&&!$_REQUEST['uid'] || $_REQUEST['uname']!=$_REQUEST['olduname']){
			$uid = M("members")->getFieldByUserName(text($_REQUEST['uname']),'id');
			$map['b.borrow_uid'] = $uid;
			$search['uid'] = $map['b.borrow_uid'];
    ...
    public function _editFilter($id)
    {
		...
		switch(strtolower(session('listaction'))){
			case "waitverify":
				for($i=0;$i<=10;$i++){
					if(in_array($i,array("1","100")) ) continue;
					unset($borrow_status[$i]);
				}
			break;
			case "waitverify_1":							//yjy20140824
				for($i=0;$i<=100;$i++){
	...
	public function doEditWaitverify(){   //执行初审yjy20140824
		//echo $this->getActionName();exit;
        $m = D(ucfirst($this->getActionName())); //borrow->Borrow
        if (false === $m->create()) {
            $this->error($m->getError());
        }
		$vm = M('borrow_info')->field('borrow_uid,borrow_status,borrow_type,first_verify_time,password,updata,danbao,vouch_money,money_collect,borrow_name,borrow_money,borrow_info')->find($m->id);//yjy borrow_name,money
		if($vm['borrow_status']==100 && $m->borrow_status<>100){  //判断是否已经初审过yjy
...
// 		debug($vm);
		...
		if($vm['borrow_status']<>2 && $m->borrow_status==2){
		  ...
		  */
			$vss = M("members")->field("user_phone,user_name,user_email")->where("id = {$vm['borrow_uid']}")->find();
			SMStip("firstV",$vss['user_phone'],array("#USERANEM#","ID"),array($vss['user_name'],$m->id));
		}
		//if($m->borrow_status==2) $m->collect_time = strtotime("+ {$m->collect_day} days");
		if($m->borrow_status==100){ 
			$m->collect_time = strtotime("+ {$m->collect_day} days");
			//$m->is_tuijian = 1;
		}
		$m->borrow_interest = getBorrowInterest($m->repayment_type,$m->borrow_money,$m->borrow_duration,$m->borrow_interest_rate);
        //保存当前数据对象
		if($m->borrow_status==2 || $m->borrow_status==1) $m->first_verify_time = time();//第一次审核时间
		else unset($m->first_verify_time);
		unset($m->borrow_uid);
		$bs = intval($_POST['borrow_status']);		//审核状态 
        ...
        if ($result = $m->save()) { //保存成功
			...
			if($vm['borrow_status']<>2 && $_POST['borrow_status']==2 && empty($vm['password'])==true) {
				 // autoInvest(intval($_POST['id']));  //yjy 自动投标执行函数
			...
			//echo "ok";exit;
			//$this->sendemail();
			//$vss['user_email']='dai_588@163.com';
			$sm = sendemail($vss['user_email'],'初审操作成功！','您的借款申请初审操作成功！<br><br>借款项目：'.$vm['borrow_name'].', 金额：'.$vm['borrow_money'].'元, '.date("Y年m月d日 H:i:s",strtotime(time())) .'<br><br>说明：'.$vm['borrow_info'].'<br>'); //yjy20140702
	...
	public function doEditWaitverify_1(){   //执行二审yjy20140824
		...
		if($vm['borrow_status']<>2 && $m->borrow_status==2){
		  ...
		  //自动投标
			if($m->borrow_type==1){
				memberLimitLog($vm['borrow_uid'],1,-($m->borrow_money),$info="{$m->id}号标二审通过");
			}elseif($m->borrow_type==2){
				memberLimitLog($vm['borrow_uid'],2,-($m->borrow_money),$info="{$m->id}号标二审通过");
			}
			$vss = M("members")->field("user_phone,user_name,user_email")->where("id = {$vm['borrow_uid']}")->find();
			SMStip("firstV",$vss['user_phone'],array("#USERANEM#","ID"),array($vss['user_name'],$m->id));
		}
        ...
        if ($result = $m->save()) { //保存成功
			...
			//$this->sendemail();
			//$vss['user_email']='dai_588@163.com';
			$sm = sendemail($vss['user_email'],'初审操作成功！','您的借款申请初审操作成功！<br><br>借款项目：'.$vm['borrow_name'].', 金额：'.$vm['borrow_money'].'元, '.date("Y年m月d日 H:i:s",strtotime(time())) .'<br><br>说明：'.$vm['borrow_info'].'<br><br><a href=http://www.daikuan.net>[河北贷信通]</a>'); //yjy20140702
	 ...
	 public function doinvest()
    {
		...
		$field= 'bi.id bid,b.id,bi.investor_capital,bi.investor_interest,bi.invest_fee,bi.add_time,bi.is_auto,m.user_name,m.id mid,m.user_phone,b.borrow_duration,b.repayment_type,m.customer_name,b.borrow_type,b.borrow_name,v.real_name';
		$list = M('borrow_investor bi')->field($field)->join("{$this->pre}members m ON m.id=bi.investor_uid")->join("{$this->pre}borrow_info b ON b.id=bi.borrow_id")->join("{$this->pre}member_info v ON m.id=v.uid")->where($map)->limit($Lsql)->order("bi.id DESC")->select();
		$list = $this->_listFilter($list);
