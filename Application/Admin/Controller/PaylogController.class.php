...
class PaylogController extends ACommonController
{
    /**
    +----------------------------------------------------------
    * 默认操作
    ...
    */
    public function index()
    {
		...
		$count = M('member_payonline w')->where($map)->count('w.id');
		...
		$listType[-1]="不限制";
		$this->assign('type_list',$listType);
		$field= 'w.id,w.uid,w.status,w.money,w.add_time,w.tran_id,w.way,w.off_bank,w.off_way,w.deal_user,m.user_name,mi.real_name';
		$list = M('member_payonline w')->field($field)->join("lzh_members m ON w.uid=m.id")->join("lzh_member_info mi ON w.uid=mi.uid")->where($map)->order(' w.id DESC ')->limit($Lsql)->select();
