...
class TborrowController extends ACommonController
{
	...
	public function doAdd( )
	{
		...
		if($data['b_img']) $model->b_img=$data['b_img'];//省心宝展示图
		$result = $model->add();
		//
		$suo=array();
		$suo['id']=$result; 
		...
		if ($result && $result2) { //保存成功
			...
			alogs("Tborrow",$result,1,'成功执行了省心宝信息的添加操作！');//管理员操作日志
		  //成功提示
			$this->assign('jumpUrl', __URL__);
			$this->success(L('新增成功'));
		}else{
			alogs("Tborrow",$result,0,'执行省心宝信息的添加操作失败！');//管理员操作日志
			$model->rollback();
			//失败提示
			$this->error(L('新增失败'));
		}
    ...
    public function doEdit() {
		...
		if($data['b_img']) $model->b_img=$data['b_img'];//修改省心宝展示图
		$result = $model->save();
		foreach($_POST['swfimglist'] as $key=>$v){
			$row[$key]['img'] = substr($v,1);
			$row[$key]['info'] = $_POST['picinfo'][$key];
        ...
        if ($result || $result2) { //保存成功
			$model->commit();
			alogs("Tborrow",0,1,'成功执行了省心宝信息的修改操作！');//管理员操作日志
          //成功提示
            $this->assign('jumpUrl', __URL__);
            $this->success(L('修改成功'));
        } else {
			alogs("Tborrow",0,0,'执行省心宝信息的修改操作失败！');//管理员操作日志
			$model->rollback();
            //失败提示
            $this->error(L('修改失败'));
        }
