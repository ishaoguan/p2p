...
class WithdrawlogingController extends ACommonController
{
    /**
    +----------------------------------------------------------
    * 默认操作 提现处理中
	 ...
	 public function doEdit() {
        $model = D("member_withdraw");
		    ...
		    if($vo['withdraw_status']<>3 && $status==3){
			      ...
			      if( ($vo['all_money'] - $vo['second_fee'])<0 ){
				      ...
				      memberMoneyLog($vo['uid'],29,-($vo['withdraw_money']),"提现申请复审已通过,扣除实际手续费".$vo['second_fee']."元，减去冻结资金，到帐金额".($vo['withdraw_money']-$vo['second_fee'])."元",'0','@网站管理员@');
				      $model->success_money = $vo['withdraw_money'];
			      ...
			      if($vo['all_money']  <=$secondfee ){
				      ...
				      memberMoneyLog($vo['uid'],36,-$vo['withdraw_money'],"提现申请复审已通过，扣除实际手续费".$secondfee."元，到帐金额".($vo['withdraw_money']-$secondfee)."元",'0','@网站管理员@',-$secondfee);
				      $model->success_money = $vo['withdraw_money'];
			      }
        ...
        if ($result) { //保存成功
          ...
          //成功提示
		    ...
		    sendsms($um['user_phone'],$vm[0]['info']);
    }
