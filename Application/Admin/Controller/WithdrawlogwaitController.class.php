...
class WithdrawlogwaitController extends ACommonController
{
    /**
    +----------------------------------------------------------
    * 默认操作 待审核提现
	 ...
	 public function doEdit() {
		...
		if($vo['withdraw_status']<>3 && $status==3){
			...
			if($vo['all_money']  <=$secondfee ){
				...
				memberMoneyLog($vo['uid'],36,-$vo['withdraw_money'],"提现申请已通过，扣除实际手续费".$secondfee."元，到帐金额".($vo['withdraw_money']-$secondfee)."元",'0','@网站管理员@');
				$model->success_money = $vo['withdraw_money'];
			}
