...
// 					unset($path[0]);//去掉分组名
		...
		case "typelist":
// 		    debug(MODULE_NAME);
// 				if(1==1||strtolower(MODULE_NAME) == strtolower(C('DEFAULT_MODULE'))) {//如果是默认分组则去掉分组名
// 					unset($path[0]);//去掉分组名
// 					$url = implode("/",$path);
// 				}
...
//  	dump(D('Acategory')->field('parent_id,type_nid'));
// 	die;
	$data_parent = D('Common/Acategory')->field('id,type_nid')->where("id = {$data['parent_id']}")->find();
...
// 		debug($type_nid);
		...
		$xurl_tmp = explode("/",str_replace(array("index.html",".html"),array('',''),$rurl[1]));//获得组合的type_nid
...
function get_area_type_infos($area_id=0){
	...
	if(!isset($_GET['typeid'])){
		...
		$type_nid = get_area_type_leve(0,$area_id=0);//获得所有栏目自己的nid的组合 
		$rurl = explode("?",$_SERVER['REQUEST_URI']); 
...
function get_avatar($uid, $size = 'middle', $type = '') {
	...
	if(!file_exists('./'.$path)) $path = __ROOT__.'/Style/header/images/'."noavatar_$size.gif";
	return  $path;
}
/**
...
// 			F('integration','','D:/Program Files/phpstudy/WWW/new/Application/Common/Webconfig/');
		}
		return $re;
	}else{
...
// 	debug($typeu);
	foreach($data as $key=>$v){
		...
		elseif($parm['model']=='article'||($v['parent_id']==0)){
		    //newslist
		$data[$key]['turl'] = MU("Home/help/index?{$v['type_nid']}/index","typelist",array("suffix"=>$suffix));
		}
		else {
		    $data[$key]['turl'] = MU("Home/help/index?{$typeu}/{$v['type_nid']}","typelist",array("suffix"=>$suffix));
	     }
	}
		$row=array();
	$row = $data;
...
function getMinfo($uid,$field='m.pin_pass,mm.account_money,mm.back_money'){
	...
	$vm = M("members m")->field('m.*,mm.*')->join("{$pre}member_money mm ON mm.uid=m.id")->where("m.id={$uid}")->find();
	}
	return $vm;
}
...
function getMemberInfoDone($uid){
	...
	->where("m.id={$uid}")->find();
	$is_data = M('member_data_info')->where("uid={$uid}")->count("id");
	$i==0;
	if($row['mbank_id']>0){
		$i++;
...
function getMemberBorrowScan($uid){
	...
	$row=array();
	$row['tborrowOut']=$receive_interest;//省心宝借出总额
...
function sendemail($to,$subject,$body){
	...
	$smtp = new \Org\Net\smtp($smtpserver,$port,true,$smtpuser,$smtppwd,$sender); 
	$send=$smtp->sendmail($to,$sender,$subject,$body,$mailtype); 
	return $send;
}
...
"id,borrow_uid,borrow_money,borrow_interest_rate,borrow_duration,repayment_type,transfer_out,transfer_back,transfer_total,per_transfer,is_show,deadline,min_month,reward_rate,increase_rate,borrow_fee,is_jijin")->find($borrow_id);
	...
	switch($binfo['repayment_type']){
		...
		case 5://一次性还款
			...
			$endTime = strtotime(date("Y-m-d",time())." ".$datag['auto_back_time']);//省心宝自动还款时间设置
			$detailInterest = getFloatValue($investinfo['investor_interest']/$duration,2);
			$investDetail['repayment_time'] = 0;
			$investDetail['borrow_id'] = $borrow_id;
			$investDetail['invest_id'] = $invest_info_id;
		...
		if($invest_info_id && $Tinvest_defail_id){
			...
			if((intval($binfo['transfer_out'])+$num)==$binfo['transfer_total']){//如果省心宝被认购完毕，则扣除借款人借款管理费
