...
//         debug($list);
    ...
    public function detail(){
		...
		$borrowinfo = M("borrow_info bi")->field('bi.*,ac.title,ac.id as aid')->join('lzh_article ac on ac.id= bi.danbao','LEFT')->where('bi.id='.$id)->find();
// 		debug($borrowinfo);
        ...
        if($_GET['borrow_id']){
           ...
           /*foreach($list as $k=>$v){
			...
			foreach($list as $k=>$v){
				...
				echo '</div>';
                echo '<div class="c_tbjl06"><img src="'.__ROOT__.'/Style/H/images/images/wancheng2.png" width="21" height="21" /></div>';
