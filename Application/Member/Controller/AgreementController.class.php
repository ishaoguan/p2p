...
class AgreementController extends MCommonController {
 ...
 public function downfile(){
		...
		$mBorrow = M("members m")->join("{$per}member_info mi ON mi.uid=m.id")->field('mi.real_name,m.user_name,mi.uid,m.id')->where("m.id={$binfo['borrow_uid']}")->find();
		...
		$time = M('borrow_investor')->field('id,add_time')->where("borrow_id={$iinfo['borrow_id']}")->order('add_time asc')->limit(1)->find();
