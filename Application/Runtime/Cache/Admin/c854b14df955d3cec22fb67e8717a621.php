...
<div class="form2">
	<form method="post" action="/new1/index.php/Admin/Members/doEdit" onsubmit="return subcheck();">
	...
	<dl class="lineD"><dt>是否直投或定存宝会员：</dt><dd><?php $i=0;$___KEY=array ( 0 => '否', 1 => '是', ); foreach($___KEY as $k=>$v){ if(strlen("1key")==1 && $i==0){ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" checked="checked" /><?php }elseif(("key1"=="key1"&&$vo["is_transfer"]==$k)||("key"=="value"&&$vo["is_transfer"]==$v)){ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" checked="checked" /><?php }else{ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" /><?php } ?><label for="is_transfer_<?php echo ($i); ?>"><?php echo ($v); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php $i++; } ?><span id="tip_is_transfer" class="tip">选择是才可以以此会员身份在后台发布直投或者定存宝</span></dd></dl>
	...
	<!--//////////////////////////////////////////////-->
	<dl class="lineD"><dt>身份证正面图片：</dt><dd><div style="text-align:left; clear:both; overflow:hidden; width:290px; height:100px"><div id="imgDiv"></div><?php if($vo["card_img"] == ''): ?>无缩略图<?php else: ?><img src="/new1/<?php echo ($vo["card_img"]); ?>" width="100" height="100" /><?php endif; ?></div>
	</dd></dl>
	<dl class="lineD"><dt>身份证反面图片：</dt><dd><div style="text-align:left; clear:both; overflow:hidden; width:290px; height:100px"><div id="imgDiv"></div><?php if($vo["card_back_img"] == ''): ?>无缩略图<?php else: ?><img src="/new1/<?php echo ($vo["card_back_img"]); ?>" width="100" height="100" /><?php endif; ?></div>
	</dd></dl>
<!--//////////////////////////////////////////////-->
	</div><!--tab1-->
