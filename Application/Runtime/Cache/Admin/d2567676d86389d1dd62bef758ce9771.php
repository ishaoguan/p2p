...
<body>
<script type="text/javascript" src="/new1/Style/A/js/uploadPreview.js"></script>
...
function searchFriend(s) {
  ...
  <?php if(is_array($friend_list)): $i = 0; $__LIST__ = $friend_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="area_<?php echo ($vo["id"]); ?>">
        ...
        <td><div style="float:left;" id="img_<?php echo ($vo['id']); ?>"><?php if(strlen($vo['link_img']) > 3) echo '<img src="/new1/'.$vo['link_img'].'" width="50" height="50" alt="'.$vo['attr_title'].'" />';else echo '无图'; ?></div></td>
...
<script type="text/javascript">
    ...
    function del_f(aid) {
		...
		$.post('/new1/index.php/Admin/Global/doDeleteFriend', datas,delResponseF,'json');
    }
