...
<!--搜索/筛选会员-->
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td><?php echo ($vo["has_borrow"]); ?></td>
        <td><?php echo ($vo["last_money"]); ?></td>
        <td><?php echo ($vo["borrow_min"]); ?></td>
		<td>
		<select name="mid" id="mid"  onblur="getusername(this);" class="c_select"><option value="">--请选择--</option><?php foreach($userlist as $key=>$v){ if($vo["mid"]==$key && $vo["mid"]!=""){ ?><option value="<?php echo ($key); ?>" selected="selected"><?php echo ($v); ?></option><?php }else{ ?><option value="<?php echo ($key); ?>"><?php echo ($v); ?></option><?php }} ?></select></td>
        <td>
...
function doMoneyComplete(){
	...
	var lastmoney = <?php echo ($vo["last_money"]); ?>;
	var datas = {'id':id,'lastmoney':lastmoney,'uid':uid};
	$.post('/new1/index.php/Admin/Borrow/doMoneyComplete', datas,Response,'json');
 }
