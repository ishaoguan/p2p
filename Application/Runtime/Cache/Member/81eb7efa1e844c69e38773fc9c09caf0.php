...
<div class="operaframe">
	<ul id="formTb">
		...
		<li style="width: 240px;">
			<input type="text" id="start_time8" value="<?php if($search['start_time8']){echo date('Y-m-d',$search['start_time8']);} ?>" readonly="readonly" class="Wdate timeInput_Day" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'end_time8\')||\'2020-10-01\'}'})"/>
			至
			<input type="text" value="<?php if($search['end_time8']){echo date('Y-m-d',$search['end_time8']);} ?>" id="end_time8" readonly="readonly" class="Wdate timeInput_Day" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'start_time8\')||\'2020-10-01\'}'})"/>
		</li>
		<li style="width: 120px;">
			<img alt="" src="/new1/Style/M/images/chakan.jpg" id="btn_search" onclick="sdetail8()" style="cursor: pointer;">
		</li>
	</ul>
</div>
<div style="margin-top: 20px; overflow: hidden; text-align: left;">
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="nodatashowtr">
		...
		<td class="tdContent"><?php echo ($vo["borrow_duration"]); if($vo["borrow_type_num"] == 1): ?>天<?php else: ?>个月<?php endif; ?></td>
		<td class="tdContent"><?php echo (date("Y-m-d",$vo["add_time"])); ?></td>
		<td class="tdContent"><?php echo ($vo['borrow_money']+$vo['borrow_interest']); ?>元</td>
		<td class="tdContent"><a href="/index.php/member/borrowdetail?id=<?php echo ($vo["id"]); ?>#fragment-1">还款明细</a></td>
	</tr><?php endforeach; endif; else: echo "" ;endif; ?>
	</tbody></table>
	<div data="fragment-8" id="pager" style="float: right; text-align: right; width: 500px; padding-right: 0px;" class="yahoo2 ajaxpagebar"><?php echo ($pagebar); ?></div>
</div>
...
function sdetail8(){
	...
	$.ajax({
		url: "/new1/index.php/Member/Borrowin/borrowdone",
		data: x,
		timeout: 5000,
		cache: false,
		type: "get",
