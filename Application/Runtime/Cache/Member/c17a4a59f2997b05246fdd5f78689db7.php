...
<div class="operaframe">
	<ul id="formTb">
		...
		<li style="width: 120px;">
			<img alt="" src="/new1/Style/M/images/chakan.jpg" id="btn_search" onclick="sdetail1()" style="cursor: pointer;">
		</li>
	</ul>
</div>
<div style="margin-top: 20px; overflow: hidden; text-align: left;">
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="nodatashowtr">
		...
		<td class="tdContent"><?php echo ((isset($vo["expired_money"]) && ($vo["expired_money"] !== ""))?($vo["expired_money"]):0.00); ?></td>
		<td class="tdContent"><?php echo ((isset($vo["call_fee"]) && ($vo["call_fee"] !== ""))?($vo["call_fee"]):0.00); ?></td>
...
function repayment(bid,sort_order){
	...
	$.ajax({
		url: "/new1/index.php/Member/Borrowin/doexpired",
		data: x,
		timeout: 15000,
		cache: false,
		type: "post",
...
function sdetail1(){
	...
	$.ajax({
		url: "/new1/index.php/Member/Borrowin/borrowbreak",
		data: x,
		timeout: 5000,
		cache: false,
		type: "get",
