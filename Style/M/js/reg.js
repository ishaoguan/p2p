...
function BlurUName() {
    ...
    if (isLegal(str) && strlen>=4 && strlen<=20) {
        ...
        $.ajax({
            type: "post",
            async: false,
            url: "/index.php/member/common/ckuser/",
			dataType: "json",
            data: {"UserName":str},
            timeout: 3000,
            success: AsyncUname
...
function BlurRec() {
	...
	if (isLegal(str) && strlen>=3 && strlen<=20) {
		...
		$.ajax({
			type: "post",
			async: false,
			url: "/index.php/member/common/ckInviteUser/",
			dataType: "json",
...
function BlurEmail() {
    ...
    if (pat.test(str)) {
        ...
        $.ajax({
            ...
            async: false,
			dataType: "json",
            url: "/index.php/member/common/ckemail/",
            data: {"Email":str},
            timeout: 3000,
            success: AsyncEmail
        });
...
function BlurCode() {
    ...
    if (pat.test(str)) {
        //格式正确
        $.post("/index.php/member/common/ckcode/", { Action: "post", Cmd: "CheckVerCode", sVerCode: str }, AsyncVerCode);
    }
    else {
		...
		success: function (d, s, r) {
			if(d){
				if(d.status==0){
					...
					window.location.href="/index.php/member/common/register2/";
