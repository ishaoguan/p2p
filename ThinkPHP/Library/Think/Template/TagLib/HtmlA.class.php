...
//         die;
     ...
     * @return string|void
     +----------------------------------------------------------
     */
    public function _commonBtn($tag)
    {
...
//         $tag        = $this->parseXmlAttr($attr,'user');
	...
	public function _tixianing($tag)
    {
//         $tag        = $this->parseXmlAttr($attr,'user');
        $uid      	= $tag['id'];                //文字
        $uname      = $tag['uname'];                //样式名
		...
		if ($uid){
		    ...
		    $parseStr = '<a onclick="loadTixianing({$_'.$uid.'},\'{$_'.$uname.'}\')" href="javascript:void(0);">编辑</a>';
		}
		
        return $parseStr;
    }

public function _tixianwait($tag)
    {
//         $tag        = $this->parseXmlAttr($attr,'user');
        $uid      	= $tag['id'];                //文字
        $uname      = $tag['uname'];                //样式名
		...
		$parseStr = '<a onclick="loadTixianwait({$'.$uid.'},\'{$'.$uname.'}\')" href="javascript:void(0);">编辑</a>';
		    ...
		    $parseStr = '<a onclick="loadTixianwait({$_'.$uid.'},\'{$_'.$uname.'}\')" href="javascript:void(0);">编辑</a>';
		}
        return $parseStr;
    }
/*****************************新增提现编辑信息扩展功能 添加人：fanyelei 添加时间：2012-12-02 09:10**********************/
